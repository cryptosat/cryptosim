...
class GroundStationNetwork {
  ...
  /**
   * @param {Station} station - a new ground station to evaluate.
   * @throws an error if the universe of the new station is different thatn the
   * the rest of the ground stations in the network.
   */
  #assertConsistentUniverse(station) {
    const universes = new Set(Object.values(
        this.#stations).map((s) => {
      return s.universe();
    }));
    universes.add(station.universe());
    if (universes.size != 1) {
      throw new Error('Station with id: \'' + station.id() +
        '\' exists in a different universe than other stations ' +
        'in the network.');
    }
  }
  ...
  addStation(station) {
    ...
    this.#assertConsistentUniverse(station);
