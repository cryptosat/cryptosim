...
class GroundStationNetwork {
  ...
  /**
   * Export the network.
   * @return {String} a serialization of the ground station network in
   *         JSON format.
   */
  toJson() {
    return {
  ...
  /**
   * Import a network.
   * @param {String} blob - a serialization of the network as exported by the
   *        toJson() function.
   */
  static fromJson(blob) {
    data = JSON.parse(blob);
    const gsnetwork = new GroundStationNetwork(data.id);
    data.stations.forEach((stationBlob) => {
      gsnetwork.addStation(GroundStation.fromJson(stationBlob));
    });
  }
};
