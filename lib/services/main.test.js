...
test('request private randomness', () => {
  ...
  const encryptedRandom = binary.ascii2ab(response.encryptedRandom);
  const signature = binary.ascii2ab(response.signature);
  expect(nacl.sign.detached.verify(
...
test('request signature', () => {
  ...
  expect(response.requestId).toEqual('id');
  const signature = binary.ascii2ab(response.signature);
  const timestamp = binary.intToArrayBuffer(response.timestamp);
