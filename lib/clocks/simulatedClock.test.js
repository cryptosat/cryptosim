...
test('invoke callbacks', async () => {
  ...
  const clock = new SimulatedClock(startDate);
  let x = 1;
  clock.callbackIn(100, () => { x += 1; });
  clock.callbackIn(110, () => { x += 2; });
  expect(x).toBe(1);
  clock.advance(100);
  expect(x).toBe(2);
  clock.advance(100);
...
test('callback using callbackAt', async () => {
  ...
  const clock = new SimulatedClock(startDate);
  let x = 1;
  clock.callbackAt(callbackDate, () => { x = 2; });
  expect(x).toBe(1);
...
test('invoking callbacks works with setDate', async () => {
  ...
  const clock = new SimulatedClock(startDate);
  let x = 1;
  clock.callbackIn(60 * 60 * 1000, () => { x = 2; });
  expect(x).toBe(1);
  clock.setDate(newDate);
  expect(x).toBe(2);
});
...
test('callbacks are called in order', () => {
  ...
  const clock = new SimulatedClock(startDate);
  let array = []
  clock.callbackIn(3, () => { array.push(3); });
  clock.callbackIn(1, () => { array.push(1); });
  clock.callbackIn(2, () => { array.push(2); });
  clock.advance(10);
