...
const DelayEncryption = require('../delay_encryption.js')
...
class MainClient {
  ...
  constructor(universe, satellite, gsnetwork, serviceId) {
    ...
    this.#delay_enc_serv = DelayEncryption();
  }
  ...
  createDelayedKeypair(time_to_release) {
    return this.#delay_enc_serv.create_keypair(time_to_release);
  }

  deleteDelayedKeypair(id) {
    return this.#delay_enc_serv.delete_keypair(id);
  }

  fetchDelayedPubKey(id) {
    return this.#delay_enc_serv.fetch_public_key(id);
  }

  fetchDelayedPrivKey(id) {
    return this.#delay_enc_serv.fetch_private_key(id);
  }
