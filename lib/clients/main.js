...
class MainClient {
  ...
  #sendRequest(msg, type, callback) {
    ...
    var nextWindow =
      this.#gsnetwork.nextTransmissionWindow(this.#satellite[0]) -
      this.#universe.clock().now();
    
    for(var i = 1; i < this.#satellite.length; i++) {
      const window =
      this.#gsnetwork.nextTransmissionWindow(this.#satellite[i]) -
      this.#universe.clock().now();
