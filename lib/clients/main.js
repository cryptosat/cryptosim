...
// if browser environment, use the browser version of the cryptosat private voting library.

const { Ballot } = require("@cryptosat/private-voting");
...
class MainClient {
  ...
  constructor(universe, satellite, gsnetwork, serviceId) {
    ...
    this.#satellite[this.#satellite_count++] = satellite;
  ...
  #sendRequest(msg, type, callback) {
    ...
    const nextWindow = this.#getNextSatellitePass();
  ...
  #getNextSatellitePass() {
    var nextWindow = this.#gsnetwork.nextTransmissionWindow(this.#satellite[0]) -
            this.#universe.clock().now();

    //var satellite = this.satellite[0];

    for(var sat of Object.values(this.#satellite)) {
      const window = this.#gsnetwork.nextTransmissionWindow(sat) -
      this.#universe.clock().now();
