 ...
 * requests are made to the API the service returns a Tracker object that
...
class MainClient {
   ...
   *        response.
   * @return {Tracker}  a tracker object used to track the status of the
   *        request.
   */
  #sendRequest(msg, callback) {
    ...
    return new Tracker(this, requestId);
  }
  ...
  #receive(stationId, msg) {
    ...
    if (body && body.requestId !== undefined) {
      const request = this.#requests.get(body.requestId);
      if (request) {
        request.external.result = request.internal.callback(body);
        request.external.status = Status.READY;
      }
    } else if (body && body.status !== undefined) {
   ...
   * @return {Tracker} -  a tracking object to followup on the status of the
  ...
  getPrivateRandom(publicKey, nonce) {
    ...
    return this.#sendRequest(msg, callback);
  }
  ...
  sign(msg) {
    ...
    return this.#sendRequest(body, callback);
  }
};
