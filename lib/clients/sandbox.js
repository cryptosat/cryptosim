  ...
  async createDelayedKeypair(timeToRelease) {
    ...
    const update = async () => {
      ...
      if (res.response.statusCode === 200) {
        ...
        const pubKeyBuffer = pubKey.export({format: 'der', type: 'spki'});
        return {
          status: 'READY',
          result: {
  ...
  async fetchDelayedPrivKey(keypairId) {
    const update = async () => {
      ...
      if (res.response.statusCode === 200) {
        ...
        const pubKey = crypto.createPrivateKey({
          key: privateKeyPem,
          format: 'pem',
        });
        const privKeyBuffer = pubKey.export({format: 'der', type: 'pkcs8'});

        return {
          status: 'READY',
          result: privKeyBuffer,
        };
      }
