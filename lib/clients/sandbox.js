...
class SandboxClient {
  ...
  /**
   * @return {Promise<string>} the cryptosat public key used for signing
   *         that was last reported by the satellite.
   *         As opposed to other methods in this client,
   *         the timestamp is not reported here as the public key is
  ...
  /**
   * @return {Promise<{timestamp: Number, signature: Buffer}>}
   *         a timestamp signed by
  ...
  /**
   * @return {Promise<{publicRandom: Buffer, signature: Buffer}>} -
   ...
   *         a tracking object to followup on the status of the
  ...
  ballot = {
    init: async (k) => {
      ...
      this.#cache.ballot = {ballotId: response.ballotId, finalized: false};
      const update = async () => {
        const res = await this.#privateBallotApi
          .ballotsBallotIdGetWithHttpInfo(this.#cache.ballot.ballotId);

        if (res.response.statusCode === 200) {
          return {
            status: 'READY',
            result: {
              ballot_id: this.#cache.ballot.ballotId,
    ...
    vote: async (encryptedVote) => {
      ...
      await this.#privateBallotApi.ballotsBallotIdVotePost(
        body,
        this.#cache.ballot.ballotId,
      );
    },

    finalize: async () => {
      if (!this.#cache.ballot.finalized) {
        await this.#privateBallotApi.ballotsBallotIdFinalizePost(
          this.#cache.ballot.ballotId,
        );
        this.#cache.ballot.finalized = true;
      }

      const update = async () => {
        const res = await this.#privateBallotApi
          .ballotsBallotIdResultGetWithHttpInfo(this.#cache.ballot.ballotId);
